...
\begin{figure}
    \includegraphics[width=13.2cm]{image1}
...
\begin{exampleblock}{Generating Dobble Decks (contd.)}
    ...
    \begin{enumerate}
        ...
        \item Find which points are incident to which lines (using the property of duality). We shall treat the three-tuples in $\mathbb{P}^2_{\mathbb{F}_9}$ as vectors in $\mathbb{R}^3$: the incidences of the points and lines are then given for a position vector $P = \langle a,b,c \rangle$ and a line $\overrightarrow{L} = \langle 0,0,0 \rangle + \mu \langle \alpha,\beta,\gamma \rangle$ such that $P \cdot \underline{\ell} = 0 \mod g(x) \mod p$.
